function [outputArg1,outputArg2] = plot_seq_instat(seq,tmp)
...
figure
subplot(3,4,[1,2]), imshow(td_conf_conv{1}(:,:,1)), ...
    grid on, xticks(0.5:1:(size(td_conf_conv{1},1)+0.5)), xticklabels([]), yticks(0.5:1:(size(td_conf_conv{1},2)+0.5)), yticklabels([]), ...
    axis on
subplot(3,4,[3,4]), imshow(td_conf_conv{1}(:,:,2)), ...
    grid on, xticks(0.5:1:(size(td_conf_conv{1},1)+0.5)), xticklabels([]), yticks(0.5:1:(size(td_conf_conv{1},2)+0.5)), yticklabels([]), ...
    axis on
subplot(3,4,5), imshow(dd_conf_conv{2}(:,:,1,1)), ...
    grid on, xticks(0.5:1:(size(td_conf_conv{2},1)+0.5)), xticklabels([]), yticks(0.5:1:(size(td_conf_conv{2},2)+0.5)), yticklabels([]), ...
    axis on
subplot(3,4,6), imshow(dd_conf_conv{2}(:,:,1,2)), ...
    grid on, xticks(0.5:1:(size(td_conf_conv{2},1)+0.5)), xticklabels([]), yticks(0.5:1:(size(td_conf_conv{2},2)+0.5)), yticklabels([]), ...
    axis on
subplot(3,4,7), imshow(dd_conf_conv{2}(:,:,2,1)), ...
    grid on, xticks(0.5:1:(size(td_conf_conv{2},1)+0.5)), xticklabels([]), yticks(0.5:1:(size(td_conf_conv{2},2)+0.5)), yticklabels([]), ...
    axis on
subplot(3,4,8), imshow(dd_conf_conv{2}(:,:,2,2)), ...
    grid on, xticks(0.5:1:(size(td_conf_conv{2},1)+0.5)), xticklabels([]), yticks(0.5:1:(size(td_conf_conv{2},2)+0.5)), yticklabels([]), ...
    axis on
...
sgtitle('Input stationary, Stride 1x1')

print(['gif/' 'instat' num2str(cnt,'%03d')], '-dpng')
close all;
cnt=cnt+1;
end

% create video
imageNames = dir(fullfile('gif','instat*.png'));
imageNames = {imageNames.name}';
outputVideo = VideoWriter(fullfile('gif','instat'), 'MPEG-4');
