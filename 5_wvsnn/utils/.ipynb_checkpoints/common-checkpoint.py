...
class WVCNN4(nn.Module):
    def __init__(self):
        ...
        self.conv1_beforenorm_weight = nn.Parameter(torch.zeros(self.conv1.weight.shape),requires_grad=False)
        self.conv1_beforenorm_bias = nn.Parameter(torch.zeros(self.conv1.bias.shape),requires_grad=False)
        self.conv2_beforenorm_weight = nn.Parameter(torch.zeros(self.conv2.weight.shape),requires_grad=False)
        self.conv2_beforenorm_bias = nn.Parameter(torch.zeros(self.conv2.bias.shape),requires_grad=False)
        self.conv3_beforenorm_weight = nn.Parameter(torch.zeros(self.conv3.weight.shape),requires_grad=False)
        self.conv3_beforenorm_bias = nn.Parameter(torch.zeros(self.conv3.bias.shape),requires_grad=False)
        self.conv4_beforenorm_weight = nn.Parameter(torch.zeros(self.conv4.weight.shape),requires_grad=False)
        self.conv4_beforenorm_bias = nn.Parameter(torch.zeros(self.conv4.bias.shape),requires_grad=False)
        self.fc_beforenorm_weight = nn.Parameter(torch.zeros(self.fc.weight.shape),requires_grad=False)
        self.fc_beforenorm_bias = nn.Parameter(torch.zeros(self.fc.bias.shape),requires_grad=False)
        
        self.conv1_beforequant_weight = nn.Parameter(torch.zeros(self.conv1.weight.shape),requires_grad=False)
        self.conv1_beforequant_bias = nn.Parameter(torch.zeros(self.conv1.bias.shape),requires_grad=False)
        self.conv2_beforequant_weight = nn.Parameter(torch.zeros(self.conv2.weight.shape),requires_grad=False)
        self.conv2_beforequant_bias = nn.Parameter(torch.zeros(self.conv2.bias.shape),requires_grad=False)
        self.conv3_beforequant_weight = nn.Parameter(torch.zeros(self.conv3.weight.shape),requires_grad=False)
        self.conv3_beforequant_bias = nn.Parameter(torch.zeros(self.conv3.bias.shape),requires_grad=False)
        self.conv4_beforequant_weight = nn.Parameter(torch.zeros(self.conv4.weight.shape),requires_grad=False)
        self.conv4_beforequant_bias = nn.Parameter(torch.zeros(self.conv4.bias.shape),requires_grad=False)
        self.fc_beforequant_weight = nn.Parameter(torch.zeros(self.fc.weight.shape),requires_grad=False)
        self.fc_beforequant_bias = nn.Parameter(torch.zeros(self.fc.bias.shape),requires_grad=False)
    ...
    def update_weights_with_quant(self,b):
        self.conv1_beforequant_weight = self.conv1.weight
        self.conv1_beforequant_bias = self.conv1.bias
        self.conv2_beforequant_weight = self.conv2.weight
        self.conv2_beforequant_bias = self.conv2.bias
        self.conv3_beforequant_weight = self.conv3.weight
        self.conv3_beforequant_bias = self.conv3.bias
        self.conv4_beforequant_weight = self.conv4.weight
        self.conv4_beforequant_bias = self.conv4.bias
        self.fc_beforequant_weight = self.fc.weight
        self.fc_beforequant_bias = self.fc.bias
    ...
    def update_weights_with_norm_values(self):
        ...
        self.conv1_beforenorm_weight = self.conv1.weight
        self.conv1_beforenorm_bias = self.conv1.bias
        self.conv2_beforenorm_weight = self.conv2.weight
        self.conv2_beforenorm_bias = self.conv2.bias
        self.conv3_beforenorm_weight = self.conv3.weight
        self.conv3_beforenorm_bias = self.conv3.bias
        self.conv4_beforenorm_weight = self.conv4.weight
        self.conv4_beforenorm_bias = self.conv4.bias
        self.fc_beforenorm_weight = self.fc.weight
        self.fc_beforenorm_bias = self.fc.bias
        
        
        w_ = self.conv1_beforenorm_weight.detach() * (self.c1_lmbda/self.input_lmbda)
        self.conv1.weight = nn.Parameter(w_,requires_grad=False)
        ...
        w_ = self.conv2_beforenorm_weight.detach() * (self.c2_lmbda/self.c1_lmbda)
        self.conv2.weight = nn.Parameter(w_,requires_grad=False)
        ...
        w_ = self.conv3_beforenorm_weight.detach() * (self.c3_lmbda/self.c2_lmbda)
        self.conv3.weight = nn.Parameter(w_,requires_grad=False)
        ...
        w_ = self.conv4_beforenorm_weight.detach() * (self.c4_lmbda/self.c3_lmbda)
        self.conv4.weight = nn.Parameter(w_,requires_grad=False)
        ...
        w_ = self.fc_beforenorm_weight.detach() * (self.fc_lmbda/self.c4_lmbda)
        self.fc.weight = nn.Parameter(w_,requires_grad=False)
...
class DWTnNorm(object):
    ...
    def __init__(self, dwt_lvl, lmbda, dlta,quant=False,b=12):
        assert isinstance(dwt_lvl, int)
        self.dwt_lvl = dwt_lvl
        self.lmbda = lmbda
        self.dlta = dlta
...
def eval_model(model, dl, print_first=True):
    correct = 0
    total = 0
